...
public class FreezeService {
    ...
    private final Map<UUID, FreezeSession> sessionsByTarget = new HashMap<>();
    ...
    public FreezeResult freeze(Player staff, Player target) {
        ...
        FreezeSession session = createSession(staff, target);
    ...
    public UnfreezeResult unfreeze(Player staff, Player target) {
        FreezeSession session = sessionsByTarget.get(target.getUniqueId());
        if (session == null) {
    ...
    public void handleQuit(Player player) {
        ...
        FreezeSession targetSession = sessionsByTarget.get(playerId);
    ...
    private FreezeSession createSession(Player staff, Player target) {
    ...
    private void handleTargetQuit(FreezeSession session, Player target) {
        removeSession(session);
    ...
    private void handleStaffQuit(Player staff, UUID targetId) {
        FreezeSession session = sessionsByTarget.get(targetId);
        if (session == null) {
            return;
        }
        removeSession(session);
    ...
    private int scheduleTargetTitleTask(FreezeSession session, Player target, Player staff) {
    ...
    private int scheduleStaffTitleTask(FreezeSession session, Player staff) {
        if (staff == null) {
            return -1;
        }
    ...
    private int scheduleTargetMessageTask(FreezeSession session, Player target, Player staff) {
